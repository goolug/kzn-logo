...
// An instance page may bring its own comp box via window.KZN_INTRO.
const INTRO = window.KZN_INTRO || { insets: [53, 138, 130, 138], fit: 'mark' };
...
const DEFAULTS = {
  ...
  ink: '77B3E6', alpha: 0.09, blend: 'multiply', soft: 0, fuse: 0.3,
  fog: 0.85, fogspeed: 0.39, fogscale: 0.65, fogdensity: 0.25,
  bokeh: 0.45,
  rays: 0.7, raymode: 'dodge', raycolor: 'FF4013', rayink: 0.58,
  rayx: 0.5, rayy: 0.44,
...
// a page may land with the panel tucked away (window.KZN_PANEL = false);
// the corner link and ?panel=0/1 override either way
const PANEL_DEFAULT = window.KZN_PANEL ?? true;
state.panel = params.has('panel') ? params.get('panel') === '1' : PANEL_DEFAULT;
...
function syncURL() {
  ...
  for (const [k, v] of Object.entries(state)) {
    if (k === 'order') {
      ...
      if (v !== PANEL_DEFAULT) p.set(k, v ? '1' : '0');
